...
class TestimonialForm {
    ...
    init() {
        ...
        window.addEventListener('resize', () => this.handleInputHeight());
    }
    ...
    handlePhotoSelect(event) {
        ...
        if (!file.type.startsWith('image/')) {
            ...
            this.showErrorToast('⚠️ Please select a valid image file (JPG, PNG, or GIF)');
            this.removePhoto();
            return;
        }
        ...
        if (file.size > maxSize) {
            ...
            this.showErrorToast(`⚠️ Photo too large (${fileSizeMB}MB). Max size is 5MB`);
            this.removePhoto();
            return;
        }
    ...
    showErrorToast(message) {
        ...
        toast.innerHTML = message; // Remove extra icon since message already has one
        ...
        // Auto-hide after 3 seconds (3000ms visible + 500ms fade out)
        setTimeout(() => {
            this.closeErrorToast();
        }, 3000);
    }
    ...
    closeErrorToast() {
        const toast = document.getElementById('fileErrorToast');
        if (toast) {
            toast.classList.remove('show');
            // Remove from DOM after animation (matching CSS transition time)
            setTimeout(() => {
                if (toast.parentNode) {
    ...
    displayPhotoPreview(file) {
        ...
        reader.onerror = (e) => {
            ...
            this.showErrorToast('⚠️ Failed to load image preview. Please try again');
        };
    ...
    validateForm(data) {
        ...
        if (!data.consent) {
            errors.push('Please consent to having your testimonial and photo displayed on this website');
        }
    ...
    showSuccess() {
        ...
        // Use the success message content from HTML template without modification
        // The HTML template already contains the intended message
    ...
    handleInputHeight() {
        const inputs = ['#name', '#role'].map(sel => document.querySelector(sel));
        inputs.forEach(input => {
            ...
            temp.style.cssText = `position:absolute;visibility:hidden;white-space:pre-wrap;word-wrap:break-word;padding:${style.padding};font:${style.font};width:${input.clientWidth}px`;
            temp.textContent = placeholder;
            document.body.appendChild(temp);
            
            if (temp.scrollHeight > input.clientHeight) {
                input.classList.add('expanded-placeholder');
            } else {
                input.classList.remove('expanded-placeholder');
            }
