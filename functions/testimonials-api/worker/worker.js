...
async function handleGetAllTestimonials(request, env, corsHeaders) {
  ...
  try {
    const stmt = env.DB.prepare(`
      SELECT id, name, role, testimonial, feedback, email, photo_url, created_at, approved
      FROM testimonials 
...
async function handleUpdateTestimonial(testimonialId, request, env, corsHeaders) {
  // Require authentication
  const isAuthenticated = await requireAuth(request, env);
  if (!isAuthenticated) {
    return new Response(JSON.stringify({ error: 'Authentication required' }), {
  ...
  try {
    const { name, role, testimonial } = await request.json();
    
    // Validation
    if (!name || !role || !testimonial) {
      return new Response(JSON.stringify({ error: 'Name, role, and testimonial are required' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
    }
    ...
    if (trimmedName.length < 1 || trimmedRole.length < 1 || trimmedTestimonial.length < 1) {
      return new Response(JSON.stringify({ error: 'All fields must contain valid content' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
    }
    ...
    const checkStmt = env.DB.prepare('SELECT id FROM testimonials WHERE id = ?');
    const existingTestimonial = await checkStmt.bind(testimonialId).first();
    ...
    const updateStmt = env.DB.prepare(`
      UPDATE testimonials 
      SET name = ?, role = ?, testimonial = ?, updated_at = CURRENT_TIMESTAMP 
      WHERE id = ?
    `);
    
    const result = await updateStmt.bind(trimmedName, trimmedRole, trimmedTestimonial, testimonialId).run();
    
    if (!result.success) {
      throw new Error('Failed to update testimonial in database');
    }
    
    console.log(`Testimonial ${testimonialId} updated successfully`);
...
async function handlePostTestimonial(request, env, corsHeaders) {
  try {
    ...
    const stmt = env.DB.prepare(`
      INSERT INTO testimonials (name, role, testimonial, feedback, email, photo_url, approved)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `);
    
    const dbResult = await stmt.bind(name, role, testimonial, feedback || null, email || null, photoUrl, 0).run();
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (url.pathname.match(/^\/api\/testimonials\/(\d+)$/) && request.method === 'PUT') {
        const testimonialId = url.pathname.match(/^\/api\/testimonials\/(\d+)$/)[1];
        return await handleUpdateTestimonial(testimonialId, request, env, corsHeaders);
      }
